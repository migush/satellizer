...
(function(window, angular, undefined) {
    ...
    angular.module('satellizer', [])
        ...
        .factory('SatellizerPopup', [
            ...
            'SatellizerConfig',
            'SatellizerUtils',
            function($q, $interval, $timeout, $window, config, utils) {
                var Popup = {};

                Popup.url = '';
                Popup.popupWindow = null;

                Popup.isBlocked = function(){
                    var _is_popup_blocked = function(){
                        if ((Popup.popupWindow.innerHeight > 0)==false){
                            return true;
                        } else {
                            return false;
                        }
                    ...
                    if (Popup.popupWindow) {
                        if(/chrome/.test(navigator.userAgent.toLowerCase())){
                            $timeout(function () {
                                return _is_popup_blocked();
                            },200);
                        }else{
                            Popup.popupWindow.onload = function () {
                                return _is_popup_blocked();
                            };
                        }
                    }else{
                ...
                Popup.open = function(url, name, options) {
                    ...
                    Popup.blocked = Popup.popupWindow==null ? true : Popup.isBlocked();
